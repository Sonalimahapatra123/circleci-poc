...
# cp -vr $CODEBUILD_SRC_DIR/blog circleci-poc-unicorn
cp -r $CODEBUILD_SRC_DIR/blog circleci-poc-sidekiq
# cp -vr $CODEBUILD_SRC_DIR/blog circleci-poc-cron
...
echo "going to remove unwanted files" 
cd app && rm -rf !(workers|models)
echo "unwanted files removal done"
cd ../

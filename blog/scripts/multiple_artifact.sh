...
echo $(cat Gemfile)
cd ../ && cd circleci-poc-sidekiq && bundle install --local #&& bundle exec sidekiq -d -L log/sidekiq.log
...
cd ../ && cd circleci-poc-sidekiq && bundle install --local #&& whenever --update-crontab --set environment=development
